---
title: Tracking quantum-dot tagged phosphorus across mycorrhizal networks to study resource movement, redistribution and trade - Data Analysis
output:
...
```{r,echo=F,warning=FALSE,message=F, out.width = '50%'}
load("./Models/lm_QD_per_mg_nut_level_control")
```
```{r}
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("./Figures/Figure_S7_RootBM.png")
```
...
We evaluate a linear model, and consider if it meets the assumptions of a linear model, given that we are evaluating proportional data. 
```{r,echo=F,warning=FALSE,message=F, out.width = '50%'}
...
We find that our linear model shows no sign of heteroscedasciticity or non-normal residual, indicating that for this dataset a linear model is appropriate even despite anayling proportional data. 

```{r}
summary(lm_total_root_bm_mg_ineq)
```

As expected, the associated linear model finds only small effects of inequality on root biomass, and no clear directionality. There is a potentially marginally signficant decrease in root biomass in the medium inequality treatment compared to the intercept. We will now evaluate, however, if the overall effect is signicant.  
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("./Figures/Figure_S8_root_colonisation.png")
```
...
## Figure S15 - Whole Plant Analysis - Colour Uptake
...
### Visualisation (Figure S15)

```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("./Figures/FigS15_WholePlantEqualColours.png")
```
...
## Figure S16 - Whole Plant Analysis - Colour Switching
...
### Visualisation (Figure S16)

```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("./Figures/FigS16_WholePlantColourSwitch.png")
```
...
## Figure S17 - Toxicity of QDs to plants
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("./Figures/FigS17_WholePlantToxicity.png")
```
