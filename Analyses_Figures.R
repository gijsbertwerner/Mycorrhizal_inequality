...
##This script performs all the statistical analyses presented in Whiteside et al. 2014 and generates all its figures.
...
ineq_discon<-read.csv("./Data/Ineq_disconnected_experimental_data.csv",as.is=T)
head(ineq_discon)
lapply(ineq_discon,class)
...
#Linear model
lm_non_origin_P_net_movement<-lm(net_movement~ineq_treatment,data=ineq_P_origin)
save(lm_non_origin_P_net_movement,file="./Models/lm_non_origin_P_net_movement")

#Visually check if assumptions of the model are met
residualPlot(lm_non_origin_P_net_movement)
qqPlot(lm_non_origin_P_net_movement)
#Ok, we have a clear outlier case. 

#Instead, analyse this in a non-parametric way
kruskal_non_origin_P_net_movement<-kruskal.test(net_movement~ineq_treatment,data=ineq_P_origin)
kruskal_non_origin_P_net_movement
#Very strong effect, as to be expected from the descriptive plots. Let's have a look if an appropriate GLM would confirm this

t.test(ineq_P_origin$net_movement[ineq_P_origin$ineq_treatment=="None"])
...
#Descriptives
ggstripchart(ineq_discon,x = "comp_nutrients",
...
##Visualisation
ineq_discon_fig<-ineq_discon %>% group_by(comp_nutrients) %>% 
...
save(ineq_discon,file="./Models/ineq_discon")
...
png("./Figures/Figure_S8_RootBM.png",res = 300,width = 5,height = 5,units = "in")
FigS8_Root_BM
dev.off()
...
png("./Figures/Figure_S7_root_colonisation.png",res = 300,width = 5,height = 5,units = "in")
FigS7_colon
dev.off()
...
#We are now evaluating the hyphal biomass only in the two fungal compartments, and evaluating at the compartment level
head(ineq)
ineq_fungal_bm<-ineq %>% filter(comp!="Root") %>% select(plate,ineq_treatment,comp,hyphal_bm_mg)
head(ineq_fungal_bm)

#Descriptives
ggstripchart(ineq_fungal_bm,x = "ineq_treatment",y="hyphal_bm_mg",
             add=c("mean_se"),color="comp")

#Statistcal Modelling

#Evaluate a linear mixed model. 
lme_hyphal_biomass_wealth<-lme(hyphal_bm_mg~ineq_treatment/comp,random = ~1|plate,data=ineq_fungal_bm,na.action = "na.omit")
save(lme_hyphal_biomass_wealth,file="./Models/lme_hyphal_biomass_wealth")

#Visually check if model assumptions are met
plot(lme_hyphal_biomass_wealth)
qqnorm(lme_hyphal_biomass_wealth$residuals)
qqline(lme_hyphal_biomass_wealth$residuals)
#The assumptions are clearly not met. 

#Evaluate a generalised linear mixed model. 
glmer_hyphal_biomass_wealth<-glmer(hyphal_bm_mg~ineq_treatment/comp+(1|plate),data=ineq_fungal_bm,na.action = "na.omit",
                                   family=inverse.gaussian(link="identity"))
save(glmer_hyphal_biomass_wealth,file="./Models/glmer_hyphal_biomass_wealth")
plot(glmer_hyphal_biomass_wealth)
qqnorm(resid(glmer_hyphal_biomass_wealth))
qqline(resid(glmer_hyphal_biomass_wealth))
#This is still not perfect yet, but let's see what the data visualisation tells us. 

#Figure

#Calculate statistics to create the figure
ineq_fungal_bm_stats<-ineq_fungal_bm %>% 
  group_by(ineq_treatment,comp) %>% summarise(
    mean_hyph_mg=mean(hyphal_bm_mg,na.rm = T),
    se_hyph_mg=stderr(hyphal_bm_mg))

#Create the figure
FigureHyphalBM<-ineq_fungal_bm_stats %>%  ggplot()+
  geom_col(aes(x=ineq_treatment,y=mean_hyph_mg,fill=comp),position="dodge")+
  geom_errorbar(aes(x=ineq_treatment,ymin=mean_hyph_mg-se_hyph_mg,
                    ymax=mean_hyph_mg+se_hyph_mg,group=comp),
                position=position_dodge(width=0.9),
                width=0.25)+
  ylab("Hyphal biomass (mg)")+
  xlab("Levels of Inequality")+
  scale_y_continuous(breaks = seq(from=0.0,to=0.6,by=0.1),labels = c("","0.1","","0.3","","0.5",""))+
  scale_x_discrete(labels=c("None","Medium","High"))+
  scale_fill_discrete(name="Nutrient\nCompartment")+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(),
        axis.line = element_line(colour="black"))
...
png("./Figures/Figure_S10_HyphalBMSplit.png",res = 300,width = 5,height = 5,units = "in")
FigureHyphalBM
dev.off()

#What does the associated model tell us? 
summary(glmer_hyphal_biomass_wealth)
Anova(glmer_hyphal_biomass_wealth)

save(ineq_fungal_bm,file="./Models/ineq_fungal_bm")

########S17 Whole Plant Colour Test
...
#Visualisation
FigS17_Colours<-whole_plant_equal_dat %>%  
...
png("./Figures/FigS17_WholePlantEqualColours.png",res = 300,width = 5,height = 5,units = "in")
FigS17_Colours
dev.off()
...
png("./Figures/FigS18_WholePlantColourSwitch.png",res = 300,width = 5,height = 5,units = "in")
FigS18_ColourSwitch
dev.off()
...
png("./Figures/FigS19_WholePlantToxicity.png",res = 300,width = 5,height = 5,units = "in")
FigureS19_tox
dev.off()
